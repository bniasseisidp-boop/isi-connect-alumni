<template>
  <div class="flex flex-col w-60 h-screen p-4 bg-white border-r-2 border-slate-50 text-slate-800 relative z-20 shadow-[10px_0_40px_rgba(0,0,0,0.01)]">
    ...
    <nav class="flex-1 space-y-2 overflow-y-auto pr-1 custom-scrollbar">
      <RouterLink 
        v-for="(item, index) in menuItems" 
        :key="item.to"
        :to="item.to" 
        class="group relative flex items-center px-3 py-3 rounded-[1.2rem] transition-all duration-500 hover:bg-slate-50 border-2 border-transparent hover:border-sky-100"
      ...
      >
        <!-- Icon Hub -->
        <div class="relative z-10 p-2.5 rounded-xl bg-slate-50 group-hover:bg-sky-500 transition-all duration-500 group-[.active-hub]:bg-sky-600 shadow-sm group-hover:shadow-lg group-hover:shadow-sky-500/20">
          <component 
            :is="item.icon" 
        ...
        <!-- Digital Indicator -->
        <div class="absolute right-3 w-1 h-1 rounded-full bg-sky-200 opacity-0 group-hover:opacity-100 group-[.active-hub]:bg-sky-500 group-[.active-hub]:opacity-100 group-[.active-hub]:scale-150 transition-all duration-700 shadow-[0_0_10px_#0ea5e9]"></div>
      </RouterLink>
    </nav>

    <!-- User & Data Access -->
    <div class="mt-auto pt-4 border-t-2 border-slate-50">
      <div v-if="auth.user.value" class="flex items-center mb-4 px-3 py-3 bg-slate-50 rounded-[1.5rem] border-2 border-transparent hover:border-sky-100 transition-all duration-500 group/user cursor-help">
        <div class="relative h-9 w-9 rounded-xl bg-slate-900 flex items-center justify-center font-black text-white text-base shadow-2xl group-hover/user:rotate-6 transition-all duration-500 overflow-hidden">

           <img v-if="getUserPhoto()" :src="getUserPhoto()" class="h-full w-full object-cover" />
           <span v-else>{{ auth.user.value.name.charAt(0) }}</span>
           <!-- Online status -->
           <div class="absolute -top-1 -right-1 h-3 w-3 bg-green-500 border-2 border-slate-50 rounded-full"></div>
        </div>
        <div class="ml-4 truncate">
          <p class="text-[10px] font-black text-slate-800 truncate uppercase tracking-tighter">{{ auth.user.value.name }}</p>
      ...
      <button 
        @click="handleLogout" 
        class="group flex items-center justify-center w-full px-3 py-4 rounded-[1.2rem] transition-all duration-500 bg-red-50 hover:bg-red-600 text-red-600 hover:text-white shadow-xl shadow-red-500/5 active:scale-95"
      >
