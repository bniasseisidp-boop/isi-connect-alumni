...
<template>
  <div class="fixed bottom-10 right-10 z-[200] flex flex-col items-end space-y-6">
    ...
    >
      <div v-if="messengerState.isOpen" class="w-[450px] h-[700px] bg-white rounded-[3.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.2)] border-2 border-slate-50 flex overflow-hidden relative">
        
        <!-- Sidebar Conversations -->
        <div class="w-24 bg-slate-950 flex flex-col items-center py-10 space-y-8 border-r border-white/5">
           <div v-for="conv in conversations" :key="conv.id" 
                @click="selectConversation(conv.user_one_id === auth.user.value.id ? conv.user_two : conv.user_one)"
                class="relative cursor-pointer group"
           >
              <div class="h-14 w-14 rounded-2xl bg-white/10 flex items-center justify-center overflow-hidden border-2 border-transparent transition-all hover:border-sky-500 group-hover:rotate-6"
              ...
              >
                  ...
                  <UserCircleIcon v-else class="h-8 w-8 text-white/20" />
              </div>
           ...
           <button 
             ...
             class="h-14 w-14 rounded-2xl bg-white/5 flex items-center justify-center text-white/20 hover:text-sky-400 transition-colors"
           >
        ...
        <div class="flex-1 flex flex-col bg-white">
          ...
          <div class="p-8 border-b border-slate-50 flex items-center justify-between">
            <div v-if="messengerState.activeChat" class="flex items-center space-x-4">
               <div class="h-12 w-12 bg-slate-900 rounded-xl overflow-hidden shadow-lg border border-slate-950">
               ...
               <div>
                 <p class="text-[10px] font-black text-slate-900 uppercase tracking-widest">{{ messengerState.activeChat.name }}</p>
                 <p class="text-[9px] font-bold text-green-500 uppercase tracking-widest opacity-80">CONNECTED</p>
               </div>
            </div>
            <div v-else>
            ...
            <div class="flex items-center space-x-2">
              <button v-if="messengerState.activeChat" @click="startCall" class="h-10 w-10 bg-sky-50 rounded-xl flex items-center justify-center text-sky-500 hover:bg-sky-500 hover:text-white transition-all shadow-sm">
                 <VideoCameraIcon class="h-5 w-5" />
              </button>
              <button @click="toggleMessenger" class="h-10 w-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all">
          ...
          <!-- Messages Stream -->
          <div id="chat-container" class="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar bg-slate-50/30">
            <template v-if="messengerState.activeChat">
              ...
              >
                <div class="max-w-[80%] rounded-[2rem] p-6 text-sm font-medium shadow-sm transition-all hover:shadow-md"
          ...
          <!-- Input Matrix -->
          <div v-if="messengerState.activeChat" class="p-8 border-t border-slate-50 bg-white">
            <div class="relative">
              <input 
                v-model="newMessage"
                @keyup.enter="sendMessage"
                type="text" 
                class="w-full bg-slate-50 rounded-2xl p-6 pr-16 text-sm font-bold border-2 border-transparent focus:border-sky-500 transition-all outline-none shadow-inner"
                placeholder="ÉMETTRE UN SIGNAL..."
              />
              <button @click="sendMessage" class="absolute right-4 top-1/2 -translate-y-1/2 h-10 w-10 bg-slate-950 text-white rounded-xl flex items-center justify-center hover:bg-sky-500 transition-all shadow-xl">
    ...
    <button 
      ...
      class="group relative h-20 w-20 rounded-[2.5rem] bg-slate-950 text-white flex items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:bg-sky-500 hover:-translate-y-2 transition-all duration-500 overflow-hidden"
    >
      <div v-if="messengerState.unreadCount > 0" class="absolute top-2 right-2 h-6 w-6 bg-red-500 border-4 border-slate-950 rounded-full z-10 animate-ping"></div>
      ...
      <transition mode="out-in">
        <XMarkIcon v-if="messengerState.isOpen" class="h-10 w-10" />
        <ChatBubbleLeftRightIcon v-else class="h-10 w-10 group-hover:rotate-12 transition-transform" />
      </transition>
    </button>
  </div>
</template>
